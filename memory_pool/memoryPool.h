...
namespace common {
    // this is a memory pool for objects of type T  
    template<typename T>
    // final keyword to prevent inheritance 
    class MemPool final {
        private:
        // this struct represent elements in the memory pool
        struct ObjectBlock {
            T object_;
            bool is_free = true;
        };
        // the actual storage for the memory pool 
        std::vector<ObjectBlock> store_;
        size_t next_free_index_ = 0;

        public: 
        // We do this so that these methods are not accidentally called without our knowledge
